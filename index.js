...
Text2svg.prototype.toPathData = function (text, options) {
  ...
  //td font italic hack
  var fontItalic = options.italic || false;
  // end td font italic hack
  ...
  //td font italic hack
  if (fontItalic) {
     var width  = fixToInt(x + padding.right + strokeWidth + (fontSize/5));
  } else {
...
Text2svg.prototype.toSVG = function (text, options) {
  ...
  if (fontItalic) {
    var x  = options.x || -(options.fontSize/10);
  } else {
    var x      = options.x || 0;
  }
